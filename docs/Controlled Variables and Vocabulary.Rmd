---
title: "Controlled Attributes and Variables"
...
* [Materials and Methods Attributes](#materials-and-methods-attributes)
    + [Materials and Methods Variables](#materials-and-methods-variables)
* [Site Level attributes](#site-level-attributes)
* [attributes that can be assigned to either cores or sites](#attributes-that-can-be-assigned-to-either-cores-or-sites)
    + [Core- or Site-level Controlled variables](#core--or-site-level-controlled-variables)
* [Core Level attributes](#core-level-attributes)
    + [Core Level Controlled variables](#core-level-controlled-variables)
* [Soil Depth Series Variables](#soil-depth-series-attributes)

### Overview
Individual studies and contributors will each have their own way of naming attributes, representing units, naming categorical attributes, and representing notes or observations. We propose the following standardized variables for data and metadata in order to make datasets machine readable and interoperable within our soil carbon synthesis products. Each sub-heading lists a level of data and metadata hierarchy from *depth series* to *core* to *site* to *study level* information. We also include accompanying sets of recommended controlled variables for key categorical variables. Some attributes have controlled units that we wish to keep uniform across datasets. Data that we curate will follow naming conventions outlined herein. Data that we ingest from outside sources will be converted to these conventions when being ingested into the central GitHub databased using custom-built R scripts.

### Study Authors Table
| attribute Name | Description | Data Type | Controlled Units |
|:------|:--------------------|:--------|:------------|
...
| core_position_method | Code indicating how *latitude* and *longitude* were determined. | character | [*rtk_gps*, *handheld_gps*, *other_high_resolution*, *other_moderate_resolution*, *other_low_resolution*](#core-level-controlled-variables), or other user-defined code |
...
| core_elevation_datum | The datum relative to which the core was measured | character  | Example: [*NAVD88*, *various tidal datums*, other user-defined code](#core-level-controlled-variables) |
| core_elevation_accuracy | Accuracy of *elevation* measurement, if determined and recorded | numeric  | meters |
| core_elevation_method | Code indicating how *elevation* was determined | character | [*rtk_gps*, *other_high_resolution*, *lidar_based*, *dem_based*,  or other user-defined code ](#core-level-controlled-variables)  |
...
| (other) | Other attributes could be included, but attribute name should follow good data management practices, the attribute should be described, and the data type defined, in the accompanying metadata | user-defined | user-defined |

#### Core Level Controlled variables
| attribute | Controlled variables | Description |
|:------|:--------|:----------------------------|
...
### attributes that can be assigned to either cores or sites
For the following attribute, salinity, vegetation, impacts, and management status could either be listed for each core separately or entered for the whole site. If entered at the site level, cores would inherit the attributes associated with the site.

| attribute Name | Description | Data Type | Controlled Units |
|:------|:--------------------|:--------|:------------|
| inundation_class | Code based on user's field observation or measurement indicating how often the coring location is inundated | character | [*high_marsh*, *mid_marsh*, *low_marsh*, *levee*, *back_marsh*, or other user-defined code](#core--or-site-level-controlled-variables) |
...
| salinity_class | Code based on user's field observation or measurement indicating average annual salinity  | character  | [*estuarine*, *palustrine*, *saline*, *brackish*, *fresh*, *polyhaline*, etc.](#core--or-site-level-controlled-variables) |
...
| vegetation_class | Code based on user's field observations or measurement indicating dominant wetland vegetation type  | character  | [*emergent*, *scrub_shrub*, *forest*, *seagrass* ](#core--or-site-level-controlled-variables), or other user-defined code  |
...
| attribute Name | Description | Data Type | Controlled Units |
|:------|:--------------------|:--------|:------------|
| coring_method | Code indicating what type of device was used to collect soil depth profiles.  | character | [*russian_auger*, *gouge_auger*, *hargas*, *other_shallow_corer*, *other_deep_corer*, or other user-defined ](#materials-and-methods-variables) |
...
| carbonates_removal_method | Code indicating how carbonates were removed for measuring fraction organic carbon by elemental analysis. | character | [*acid_fumigation*, *direct_acid_treatment*, or other user-defined method](#materials-and-methods-variables) |
...
|(other)| Any other user-defined attributes | user-defined | user-defined |
